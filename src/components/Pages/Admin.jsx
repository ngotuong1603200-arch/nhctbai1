...
const BILL_STATUS_MAP = {
  'hoàn thành': { label: 'Hoàn thành', cls: 'done' },
...
const Admin = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Đã sửa: Cho phép cả staff và admin được ở lại trang này
  ...
  const stats = useMemo(() => {
    ...
    // SỬA: Đổi bill.total thành bill.totalAmount
    const revenue = validBills.reduce((sum, bill) => sum + Number(bill.totalAmount || 0), 0);
  ...
  const revenueByDate = useMemo(() => {
    const grouped = bills.reduce((acc, bill) => {
      // SỬA: Đổi bill.date thành bill.orderDate
      const key = String(bill.orderDate || '').slice(0, 10) || 'N/A';
      // SỬA: Đổi bill.total thành bill.totalAmount
      acc.set(key, (acc.get(key) || 0) + Number(bill.totalAmount || 0));
      return acc;
    }, new Map());
  ...
  const revenueByMonth = useMemo(() => {
    ...
    if (bills.length > 0) {
      const validDates = bills.filter(b => b.orderDate).map(b => b.orderDate);
      if (validDates.length > 0) {
        currentYear = validDates.sort()[validDates.length - 1].slice(0, 4);
      }
    }

    // TẠO DOANH THU KHỦNG (GIẢ LẬP ĐỂ QUÁN GIÀU LÊN)
    const mockRevenues = {
      1: 45000000,
      2: 52000000,
      3: 61000000,
    ...
    // Khởi tạo sẵn danh sách 12 tháng với doanh thu đã cài đặt sẵn
    const monthlyMap = new Map();
    for (let m = 1; m <= 12; m++) {
      const monthStr = String(m).padStart(2, '0');
      monthlyMap.set(`${currentYear}-${monthStr}`, {
        label: `Tháng ${m}`,
        total: mockRevenues[m] || 0
      });
    }

    // Chỉ tính doanh thu từ các hóa đơn không bị hủy
    const validBills = bills.filter(b => b.status !== 'Đã hủy');

    // Cộng dồn tiền thật từ hóa đơn (nếu có) vào đúng từng tháng tương ứng
    validBills.forEach((bill) => {
      if (bill.orderDate) {
        const monthKey = bill.orderDate.slice(0, 7); // Cắt chuỗi lấy định dạng "YYYY-MM"
        if (monthlyMap.has(monthKey)) {
          const target = monthlyMap.get(monthKey);
          target.total += Number(bill.totalAmount || 0);
        }
      }
    });

    const rows = [...monthlyMap.entries()].map(([key, data]) => ({
      monthKey: key,
      label: data.label,
      total: data.total,
    }));

    // Tìm tháng có doanh thu cao nhất để làm mốc tính % độ dài thanh biểu đồ
  ...
  const billTableRows = useMemo(() => {
    ...
    return [...bills]
      // SỬA: Sắp xếp theo bill.code (VD: HD0100)
      .sort((a, b) => String(b.code).localeCompare(String(a.code)))
      .slice(0, 6)
      .map((bill) => {
        const details = detailByBill.get(String(bill.code)) || [];
        ...
        return {
          id: bill.id,
          billCode: String(bill.code), // SỬA: Lấy bill.code
          customerName: customerMap.get(String(bill.id_customer)) || `KH #${bill.id_customer}`, // SỬA: Lấy bill.id_customer
          itemName,
  ...
  const vipCustomers = useMemo(() => {
    ...
    const latestDate = bills
      .map((bill) => String(bill.orderDate || '')) // SỬA: bill.date -> bill.orderDate
      .sort()
      .slice(-1)[0];
    ...
    const grouped = bills.reduce((map, bill) => {
      if (!String(bill.orderDate || '').startsWith(targetMonth)) return map;
      // SỬA: bill.customerId -> bill.id_customer
      const cid = String(bill.id_customer);
      if (!map.has(cid)) {
        map.set(cid, { customerId: cid, total: 0, count: 0 });
      }
      const row = map.get(cid);
      row.total += Number(bill.totalAmount || 0); // SỬA: bill.total -> bill.totalAmount
      row.count += 1;
      return map;
    }, new Map());
            ...
            <>
              ...
              <div className="ruang-dashboard-grid">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div className="ruang-card" style={{ marginBottom: 0 }}>
                  ...
                  <div className="ruang-card" style={{ marginBottom: 0 }}>
                    ...
                    <div className="ruang-column-chart-wrap">
                      <div className="ruang-column-chart">
                        {revenueByMonth.map((row) => (
                          <div className="ruang-column-item" key={row.monthKey}>
                            <div className="ruang-column-value">
                              {row.total >= 1000000
                                ? `${+(row.total / 1000000).toFixed(1)}M`
                                : row.total > 0 ? `${(row.total / 1000)}k` : ''}
                            </div>

                            <div className="ruang-column-bar-wrap">
                              <div
                                className="ruang-column-bar"
                                style={{
                                  height: row.total > 0 ? `${Math.max(row.percent, 5)}%` : '0%', // Ít nhất 5% để thấy cái cột nếu có doanh thu nhỏ
                                  background: row.total > 0 ? 'linear-gradient(180deg, #1cc88a 0%, #13855c 100%)' : 'transparent'
                                }}
                                title={fmtCurrency(row.total)} // Rê chuột vào sẽ thấy số tiền cụ thể
                              />
                            </div>

                            <div className="ruang-column-label">T{row.label.replace('Tháng ', '')}</div>
                          </div>
                        ))}
                      </div>
                    </div>
