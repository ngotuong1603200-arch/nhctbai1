...
const Profile = () => {
 ...
 const handleChangePassword = async (e) => {
    ...
    try {
      // Gọi API để backend cập nhật mật khẩu thẳng vào account.json
      ...
      // Cập nhật lại localStorage để phiên đăng nhập hiện tại không bị văng
      const updatedUser = { ...user, password: n1 }; 
      localStorage.setItem('currentUser', JSON.stringify(updatedUser));

      setPwMsg('Đã cập nhật mật khẩu thành công!');
      clearPasswordForm();
    } catch (err) {
      setPwMsg('');
      setPwErr(err.response?.data?.error || 'Lỗi: Không thể lưu vào file (Hãy đảm bảo Backend / JSON Server đang chạy).');
    }
  };
